...
if __name__ == '__main__':
    ...
    tokent = [set(t) for t in tt] # will look up by id so in O(1)
    ...
    RUN = 0
    rdd1 = sc.parallelize(tt[RUN * 10000: RUN * 10000 + 10000], numSlices=1000)
    rdd2 = sc.parallelize(tt, numSlices=1000)
    t = rdd1.zipWithIndex().filter(lambda x: len(x[0]) > 4).map(lambda x: Row(t1=x[0],i1=x[1]))
    t2 = rdd2.zipWithIndex().filter(lambda x: len(x[0]) > 4).map(lambda x: Row(t2=x[0],i2=x[1]))
