...
**After a while Mark would notice my comments and would begin to wonder "Who is this guys that is always posting meaningful responses to all my posts?". Then he would invite my wife and me to his house for dinner and our kids will become friends. :)**
...
Oh well, I thought, there is always a next time. Sure enough within a day I had another text. This time it was within under 1 minute from the original post. I quickly open the link, only to discover that Mark's post already had close to 100 comments.
...
What I didn't realize in my estimate is that those comments were not evenly distributed in time.

At that point I began to consider my options. 

I could set my script to run more often than every 60 seconds. Then I would risk showing up on Facebook's radar as a spammer and it just didn't feel right.

Another option I considered was to try to make an automated post, in order to be one of the first people to comment. This approach, however, would defeat the purpose of saying something meaningful and would not help me to become friends with Mark. Sure, I could do it to prove the point, but I wasn't out to prove anything, I just wanted to learn.
...
Having made a big error in my estimate of the rate at which people were replying to Mark, I was curious to see explore what and when people were saying. In order to do that, I needed a data set of comments. 
...
Once again I under estimate the scale of the operation. There were just too many comments (over 20,000) and it was too much for a browser to handle. Both Firefox and PhantomJS continued to crush without being able to load all of the comments
...
I've ran the CURL command to get more comments in my terminal, and it returned some JSON.
...
I've converted the CURL command to Python request code via [this online tool](http://curl.trillworks.com/).
...
As can be seen in the two plots bellow, it looks a lot like exponential decay, with most of the comments being made in the first two hours. 
...
Looking at the word cloud and quickly scheming through the comments I realized that I might have picked the wrong day to do this experiment. Most of the people responded in kind to Mark's wishes of Merry Christmas and Happy New Year. That was, of course, great new for Mark, but kind of boring from the data exploration stand point.
...
I've recently finished reading [Data Smart](http://amzn.to/1S0lYdP), from which I learned that [Network Analysis](https://en.wikipedia.org/wiki/Network_theory) can be used to identify various data points that belong together, also known as clusters.

One of the examples in [the book](http://amzn.to/1S0lYdP) uses [Gephi](https://gephi.org/) - an amazing software that makes cluster analysis very easy and fun. I wanted to analyze the "unusual" comments in Gephi, but first I had to find a way to represent them as a Network.

In order to do that, I've:

1. Removed meaningless words such as "and" or "or" (also known as [stop words](https://en.wikipedia.org/wiki/Stop_words)) from every comment
...
I've said all intersection in the edges.cvs file, that had the following format:
...
I've also noticed some weaknesses in my approach. For example there were two clusters around the word "precious". It was probably caused by removing relationships that did not have intersection score of at least 0.3.

Please let me know in the comments, if you find anything else note worthy.
...
It would probably take a dedicated Data Scientist working full time, just to get insight into all of the comments that Mark gets. While Mark can afford to have such a dedicated person, my bet is that he is using his resources for more meaningful things.
...
If you want some inspiration for Python code that can help you perform similar automation tasks I highly recommend reading [Automate the Boring Stuff with Python](http://www.alexkras.com/review-automate-the-boring-stuff-with-python-by-al-sweigart/).

If you are looking for a good entry level text on Data Science. I found [Data Smart](http://amzn.to/1S0lYdP) to be an easy read, although hard to follow at times.


I've destroyed all of my data sets to comply as best as I can with Facebook's Terms of Service. Scraping content without permission is also against Facebook's Terms of Service, but I did not realize that I was in violation until after I've done all of my analysis. I am hoping that Facebook will over look my transgression, but wanted to make sure I don't send anybody else down the wrong path without a proper warning.
